...
`assets/`に配置できるのは、e-Statのようにアカウントがないとダウンロードできない情報だけとする。誰でもURLから直接ダウンロードできる資料は`assets/`に置かず、取得先のURLをレポート本文に記載する。
...
│   │       └── assets/                      # アカウントがないとダウンロードできないファイルのみ（無い調査では作らない）
